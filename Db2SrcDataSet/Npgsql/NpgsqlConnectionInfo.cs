...
namespace Db2Source
{
    public class NpgsqlConnectionInfo: ConnectionInfo
    {
        ...
        public override void SavePassword()
        {
            ...
            string target = string.Format("{0}:{1}:{2}:{3}:", ServerName, ServerPort, DatabaseName, UserName);
            string newEntry = target + ":" + Password;
            if (File.Exists(path))
            {
        ...
        public static ConnectionInfo[] GetKnownConnectionInfos()
        {
            ...
            using (StreamReader sr = new StreamReader(stream, Encoding.Default))
            {
                while (!sr.EndOfStream)
                {
                    ...
                    string[] prms = s.Split(new char[] { ':' }, 5);
                    if (prms.Length < 5)
                    {
                        continue;
                    }
            ...
            list.Sort(ConnectionInfo.CompareByName);
            return list.ToArray();
        }
